...
class DataSchemaScopeComputer {
	...
	def dispatch void buildDataSchemaScope(Statement statement, QualifiedName prefix, Ctx ctx) {
		...
		if (name != prefix && !ctx.scopes.getLocal(YangScopeKind.NODE).tryAddLocal(name, statement)) {
	...
	private def doLink(SchemaNodeIdentifier identifier, QualifiedName prefix, Ctx context) {
		...
		for (e : identifier.elements) {
			...
			linker.link(e, YangPackage.Literals.IDENTIFIER_REF__NODE) [
				val result = context.scopes.getFull(YangScopeKind.NODE).getSingleElement(p)
				return result
			]
		}
	}
